...
def loadFile(filename):
	...
	for line in lines:
		...
		if tokens[0] == "node":
			...
			connections.append({'status': 'closed', 'type': 'connection', "from": [int(x) for x in tokens[1].split(",")], "to": int(tokens[2]), "anchor": tokens[3], "anchoroffset": int(tokens[4]), "body": tokens[5]})
		else:
